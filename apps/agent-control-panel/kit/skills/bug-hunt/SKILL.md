---
name: bug-hunt
description: Run a multi-agent bug hunt over a test suite or project directory — parallel finders (logic, async, selectors, flakiness, config) with adversarial verification. Usage — /bug-hunt [directory], defaults to e2e/george-search.
---
...
1. Determine the target directory from the arguments. If none given, default to `e2e/george-search`. It must be a directory that exists in this repo (`e2e/george-search`, `packages/flaky-test-detector`, `packages/self-healing-tests`, `packages/visual-regression-cli`, `apps/qa-swiss-knife`, etc.). If the argument doesn't match an existing directory, list the candidates and ask.
